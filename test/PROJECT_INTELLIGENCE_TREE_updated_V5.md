...
│   │   │     - Key methods: `createIndent`, `updateIndent`, `getIndentById`, `getIndentDataForTenderById`, `getIndentDataById`, `searchIndents`, `assignEmployee`, `requestIndentCancellation`, `approveCancellationRequest`, `getIndentVersionHistory`, `saveIndentDraft`, `updateIndentDraft`, `submitIndentDraft`, `getUserDrafts`.
...
│   │   │       - `POST /api/indents/draft` — save new draft; skips all validation; generates a real indent ID immediately; returns `IndentCreationResponseDTO` with `currentStatus=DRAFT`.
│   │   │       - `PUT /api/indents/draft?indentId=` — update existing draft in-place (same ID, no versioning, clears and re-saves material/job details via `orphanRemoval=true`). Guards: status must be DRAFT, createdBy must match.
│   │   │       - `POST /api/indents/draft/submit?indentId=` — promote a draft to submitted; runs full backend validation + budget hold + workflow initiation. Same logic as fresh `POST /api/indents` but starts from the saved draft row.
│   │   │       - `GET /api/indents/drafts?userId=` — list all DRAFT indents for a user (for draft picker UI).
...
│   │   │   - `BudgetController` (`/api/admin/budget`) — MODIFIED:
│   │   │     - `updateBudget` (PUT): restricted to admin-editable fields only (`budgetName`, `category`, `allocatedAmount`, `fiscalYear`, `startDate`, `endDate`, `status`, `projectCode`, `departmentName`). `onHoldAmount` and `spentAmount` are system-managed — never set via this endpoint.
│   │   │     - Validation added: new `allocatedAmount` must not go below `onHoldAmount + spentAmount` (already committed). Returns error if violated.
│   │   │     - `getRemainingAmount()` is `@Transient` — auto-computed as `allocatedAmount - onHoldAmount - spentAmount`. Updating `allocatedAmount` automatically changes available balance.
...
│   │   │     - Key methods: `initiateWorkflow`, queue/history/action helpers, workflow branch evaluation, `approveBranchBasedTransition`, `terminateWorkflowOnRejection`, `updateRequestEntityOnRejection`.
│   │   │     - Dependencies: workflow master/transition/state/action/admin branch repositories, `BudgetService`, procurement repos (indent, PO, SO).
│   │   │     - Note: very large hotspot (~5732 lines).
│   │   │     - BUDGET HOOKS (added):
│   │   │       - `approveBranchBasedTransition` (`nextApprover == null` block — final approval):
│   │   │         - If `requestId.startsWith("IND")` → indent workflow complete, no budget action (hold already placed at submit).
│   │   │         - If `requestId.startsWith("PO")` → calls `BudgetService.finalizePoHold`: releases indent holds, places PO hold. Throws on insufficient budget — blocks final approval.
│   │   │         - If `requestId.startsWith("SO")` → calls `BudgetService.finalizeSOAsSpent`: releases indent holds, moves SO amount directly to Spent. Throws on insufficient budget — blocks final approval.
│   │   │       - `updateRequestEntityOnRejection`:
│   │   │         - `IND` prefix → calls `BudgetService.releaseHoldIfRejected(requestId, "INDENT")` — releases indent hold back to available. Error swallowed (logged only).
│   │   │         - `PO` prefix → calls `BudgetService.releaseHoldIfRejected(requestId, "PO")` — releases PO hold if it exists. Error swallowed.
│   │   │         - `SO` prefix → calls `BudgetService.releaseSOSpent(requestId, tenderId)` — reverses spent, restores indent holds. Error swallowed.
│   │   │         - Send-back / change-request path: does NOT call any budget method — hold preserved.
│   │   ├── procurement
│   │   │   - `IndentCreationServiceImpl`
│   │   │     - Purpose: build indent aggregate, validate mode/vendor count, save files, compute value, cancellation flow, indent versioning, budget hold management, draft lifecycle.
│   │   │     - Key methods: `createIndent`, `updateIndent`, `getIndentVersionHistory`, `searchIndentIds`, `assignEmployeeToIndent`, `requestIndentCancellation`, `approveCancellationRequest`, `extractBaseIndentId`, `saveIndentDraft`, `updateIndentDraft`, `submitIndentDraft`, `getUserDrafts`.
│   │   │     - Dependencies: indent/material/job repos, workflow transition repo, vendor/job mapping repo, user/employee repos, PO repos, email, `BudgetService`.
│   │   │     - Transaction boundaries: `createIndent`, `updateIndent`, `requestIndentCancellation`, `approveCancellationRequest`, `saveIndentDraft`, `updateIndentDraft`, `submitIndentDraft`.
│   │   │     - DRAFT LIFECYCLE (added):
│   │   │       - `saveIndentDraft`: saves an `IndentCreation` row with `currentStatus=DRAFT`. Skips all business validation and budget. Generates a real indent ID. Does NOT call `BudgetService` or `WorkflowService`. Returns `IndentCreationResponseDTO` with the new ID immediately.
│   │   │       - `updateIndentDraft`: finds existing DRAFT row by ID, guards on `currentStatus=DRAFT` and `createdBy` match. Overwrites all fields in-place — same indent ID, no versioning. Clears `materialDetails` and `jobDetails` via `orphanRemoval=true` then re-adds from DTO. Does NOT call budget or workflow.
│   │   │       - `submitIndentDraft`: promotes a DRAFT row to submitted — runs full validation, calls `BudgetService.holdBudgetForIndent`. Workflow is initiated by the controller after this method returns. Identical business logic to `createIndent` but starts from the pre-saved draft row.
│   │   │       - `getUserDrafts`: returns all rows for a `createdBy` userId where `currentStatus=DRAFT`.
│   │   │     - DRAFT NULL-SAFETY FIX in `mapToResponseDTO` (added — required for draft support):
│   │   │       - Line ~1975: `.filter(Objects::nonNull)` added before `.findFirst()` on `materialSubCategory` stream — draft rows may have null subCategory since the user has not selected a material yet.
│   │   │       - Line ~2028 and ~2067: `.map(p -> p != null ? p : BigDecimal.ZERO)` added before `.reduce(BigDecimal.ZERO, BigDecimal::add)` for both material and job total-price reductions — draft rows may have null `totalPrice` since qty/price may not be entered yet.
│   │   │       - These are safe additions for non-draft paths too (defensive only).
...
│   │   │     - BUDGET HOLD LOGIC (added):
│   │   │       - `createIndent`: calls `BudgetService.holdBudgetForIndent` after total price computed, before `saveAndFlush`. Groups line items by `budgetCode` (from `MaterialDetails` or `JobDetails` depending on `indentType`), checks available balance per budget, blocks submission if insufficient, otherwise places hold.
│   │   │       - `updateIndent` (versioning): calls `BudgetService.reHoldBudgetForUpdatedIndent(oldIndentId, newIndentId, ...)` before `saveAndFlush` of new version. Validation-first pass checks all budget codes — if any budget has insufficient extra funds, entire update is blocked and old hold is preserved. Apply pass runs only after all validations pass.
│   │   │       - V2 < V1 case (amount decreased): always succeeds — old hold released, new (smaller) hold placed, difference returned to available.
│   │   │       - `approveCancellationRequest`: calls `BudgetService.releaseBudgetHoldForIndent` when cancellation is approved — releases hold back to available.
...
│   │   │   - `PurchaseOrderImpl`
│   │   │     - Purpose: create/update PO aggregate, version history, lock tender after PO, reporting/format generation, PO versioning, budget check/hold management.
...
│   │   │     - Dependencies: PO repos, tender/indent repos/services, vendor/project/material repos, signature/freight/address repos, workflow transition repo, `BudgetService`.
│   │   │     - VERSIONING LOGIC (added):
...
│   │   │     - BUDGET LOGIC (added):
│   │   │       - `createPurchaseOrder`: calls `BudgetService.checkBudgetForPo` before save — validates PO total vs sum of indent holds for the tender's indents, per budgetCode. Blocks if PO exceeds indent hold and available is insufficient. Does NOT touch hold — indent holds remain until PO final approval.
│   │   │       - `updatePurchaseOrder`: calls `BudgetService.checkBudgetForPo` before saving new version — same check against current indent holds.
│   │   │       - PO final approval (in `WorkflowServiceImpl`): calls `BudgetService.finalizePoHold` — releases all indent holds for tender, places PO hold per budgetCode. Re-validates available before applying. Blocks approval if insufficient.
│   │   │       - PO rejection (in `WorkflowServiceImpl`): calls `BudgetService.releaseHoldIfRejected` — releases PO hold if it exists (post-final-approval rejection edge case).
│   │   │       - PO cancel (TODO): calls `BudgetService.cancelPoHold` — releases PO hold, restores indent holds. Blocked if available insufficient to restore indent hold after PO hold release.
│   │   │   - `ServiceOrderServiceImpl`
│   │   │     - Purpose: create/update SO aggregate from tender/job material selection, report generation, SO versioning, budget check/spend management.
...
│   │   │     - Dependencies: SO repo, workflow transition repo (newly injected), `BudgetService`.
│   │   │     - VERSIONING LOGIC (added):
...
│   │   │     - BUDGET LOGIC (added):
│   │   │       - `createServiceOrder`: calls `BudgetService.checkBudgetForSo` before save — validates SO total vs sum of indent holds for the tender, per budgetCode. Blocks if SO exceeds indent hold and available is insufficient. No hold placed — indent holds remain.
│   │   │       - `updateServiceOrder`: calls `BudgetService.checkBudgetForSo` before saving new version — same validation.
│   │   │       - SO final approval (in `WorkflowServiceImpl`): calls `BudgetService.finalizeSOAsSpent` — releases indent holds, moves SO amount DIRECTLY to Spent (no hold step). Re-validates available before applying. Blocks approval if insufficient.
│   │   │       - SO line amount formula: `(qty × rate × exchangeRate) + gst% + duties%` — no freight (SO has no freightCharge field).
│   │   │       - SO rejection (in `WorkflowServiceImpl`): calls `BudgetService.releaseSOSpent` — reverses spent amount, restores indent holds.
...
│   │   │     - BUDGET HOOK (added): `approveGrn` calls `BudgetService.convertHoldToSpentOnGrn(grnSubProcessId)`. Resolves chain: GRN → GI subProcessId → GPRN subProcessId → poId. For each accepted material line: non-consumables resolve `materialCode` via `AssetMaster.assetId`; consumables have `materialCode` directly. Spend formula: `(acceptedQty × rate × exchangeRate) + gst% + duties% + freightCharge` (flat per line). Reduces PO hold, increases spent on `BudgetMaster`. Updates `BudgetLedger` PO entry. Error swallowed — GRN approval must not fail due to budget side effects.
│   │   │   - `GiServiceImpl`
...
│   │   │     - BUDGET HOOK (added): `approveGi` calls `BudgetService.releaseHoldForPermanentRejection(inspectionSubProcessId, poId)`. Resolves poId via GI → GPRN chain. For each material/consumable line with `rejectionType = "permanent"`: computes `rejectedQty × rate × exchangeRate + gst% + duties% + freightCharge`, releases that amount from PO hold back to available. Replacement/partial rejections do NOT trigger budget release. Error swallowed.
│   │   │   - `GtServiceImpl`
...
│   │   │   - `BudgetService` (interface) + `BudgetServiceImpl` (NEW):
│   │   │     - Purpose: centralised budget lifecycle manager — hold, re-hold, release, finalize, spend across all procurement stages.
│   │   │     - Location: `com/astro/service/BudgetService.java`, `com/astro/service/impl/BudgetServiceImpl.java`.
│   │   │     - Dependencies: `BudgetMasterRepository`, `BudgetLedgerRepository`, `IndentIdRepository`, `PurchaseOrderAttributesRepository`, `GrnMasterRepository`, `GrnMaterialDtlRepository`, `GrnConsumableDtlRepository`, `GiMaterialDtlRepository`, `GoodsInspectionConsumableDetailRepository`, `GprnMasterRepository`, `AssetMasterRepository`.
│   │   │     - Interface methods:
│   │   │       - `holdBudgetForIndent(indentId, materials, jobs)` — on indent submit.
│   │   │       - `reHoldBudgetForUpdatedIndent(oldId, newId, materials, jobs)` — on indent version update.
│   │   │       - `releaseBudgetHoldForIndent(indentId)` — on indent cancellation approved.
│   │   │       - `releaseHoldIfRejected(referenceId, referenceType)` — on indent/PO rejection.
│   │   │       - `checkBudgetForPo(poId, tenderId, poAttributes)` — check only on PO create/update.
│   │   │       - `finalizePoHold(poId, tenderId, poAttributes)` — on PO final approval.
│   │   │       - `cancelPoHold(poId, tenderId)` — on PO cancel (TODO: hook point TBD).
│   │   │       - `checkBudgetForSo(soId, tenderId, soMaterials)` — check only on SO create/update.
│   │   │       - `finalizeSOAsSpent(soId, tenderId, soMaterials)` — on SO final approval.
│   │   │       - `releaseSOSpent(soId, tenderId)` — on SO rejection.
│   │   │       - `convertHoldToSpentOnGrn(grnSubProcessId)` — on GRN approval.
│   │   │       - `releaseHoldForPermanentRejection(inspectionSubProcessId, poId)` — on GI approval with permanent rejection.
│   │   │     - Budget state: `Total = Available + OnHold + Spent`. Available = `allocatedAmount - onHoldAmount - spentAmount` (computed, never stored).
│   │   │     - Line amount formula (PO/GRN): `(qty × rate × exchangeRate) + gst% + duties% + freightCharge (flat)`.
│   │   │     - Line amount formula (SO): `(qty × rate × exchangeRate) + gst% + duties%` — no freight.
│   │   │     - ExchangeRate applied only for non-INR currencies.
│   │   │   - `BudgetLedger` entity + `BudgetLedgerRepository` (NEW):
│   │   │     - Table: `budget_ledger`. Tracks per-reference hold/spend entries.
│   │   │     - Key fields: `budgetCode`, `referenceId` (indentId/poId/soId), `referenceType` (INDENT/PO/SO), `holdAmount`, `spentAmount`, `status` (ACTIVE_HOLD / RELEASED / CONVERTED_TO_SPEND).
│   │   │     - Key query: `findByReferenceIdAndTypeAndStatus(referenceId, referenceType, status)`.
│   │   └── utilities
...
│   │   ├── `ButtonContainer.jsx` (UPDATED — draft pattern):
│   │   │   - Props: `submitBtnLoading`, `submitBtnEnabled`, `onFinish`, `printBtnEnabled`, `handlePrint`, `draftDataName` (legacy, used only for localStorage clear on Reset), `draftBtnEnabled`, `draftBtnLoading` (NEW), `onDraft` (NEW), `formData`, `disabled`, `showCancel`, `onCancel`, `cancelButtonText`.
│   │   │   - CRITICAL — Submit vs Draft button difference:
│   │   │     - Submit button uses `htmlType="submit"` → triggers Ant Design `Form` validation before `onFinish` fires. User CANNOT submit without all required fields.
│   │   │     - Draft button uses `onClick={onDraft}` only — NO `htmlType="submit"`. Form validation is completely bypassed. User CAN save with any fields empty.
│   │   │   - `onDraft` replaces the old local `saveDraft` function that wrote to `localStorage`. The parent page (e.g. `Indent1.jsx`) provides `onDraft` as an API-calling handler.
│   │   │   - `draftBtnLoading` drives the loading spinner on the Draft button, fed from parent state.
│   │   │   - `handleReset` still calls `localStorage.removeItem(draftDataName)` for legacy cleanup but draft data now lives on the server.
│   │   │   - Cancel button uses Popover with remarks textarea — same as before.
│   │   │   - REUSE RULE: Any page that wants API-backed draft saving must pass `onDraft` (async API handler) and `draftBtnLoading` (boolean state). The button itself has no knowledge of which API to call.
│   │   └── queue helpers
...
│   │   │     - Key functions: search indent, load job/uom/budget/employee context, submit indent, save/update/submit draft.
...
│   │   │     - DRAFT FLOW (added):
│   │   │       - `handleSaveDraft` (new async function): builds same payload as `onFinish` but calls draft API instead of submit API. Routing logic:
│   │   │         - `formData.indentId` is null → `POST /api/indents/draft` (new draft). Backend returns a real indent ID.
│   │   │         - `formData.indentId` exists AND `formData.currentStatus === "DRAFT"` → `PUT /api/indents/draft?indentId=` (update same draft in-place).
│   │   │         - `formData.indentId` exists AND status is NOT DRAFT → shows warning "already submitted, cannot save as draft".
│   │   │       - After successful draft save: `setFormData(prev => ({ ...prev, indentId: savedData.indentId, currentStatus: "DRAFT" }))` — subsequent saves correctly route to PUT.
│   │   │       - `draftBtnLoading` state drives the loading spinner on the draft button.
│   │   │       - `onFinish` already handles DRAFT-status indents: if `currentStatus === "DRAFT"`, calls `POST /api/indents/draft/submit?indentId=` (promote draft to submitted) which runs full validation + budget + workflow.
│   │   │       - "Saved Draft" alert banner shown when loaded indent has `currentStatus === "DRAFT"`.
│   │   │       - Search dropdown shows `[DRAFT]` label suffix for draft indent IDs.
│   │   │       - `ButtonContainer` receives `onDraft={handleSaveDraft}` and `draftBtnLoading={draftBtnLoading}` — no localStorage involved.
...
- Indent flow:
  ...
  - Draft flow (added): Save Draft button bypasses form validation entirely (no `htmlType="submit"`). `handleSaveDraft` routes to `POST /api/indents/draft` (new) or `PUT /api/indents/draft` (update existing draft). After save, `formData.indentId` and `currentStatus=DRAFT` are set so subsequent saves correctly call PUT. On Submit click, if status is DRAFT, calls `POST /api/indents/draft/submit` to promote to full submission with validation + budget + workflow.
...
- `GET/POST/PUT /api/indents*`
  ...
  - Response: `IndentCreationResponseDTO` (includes `version`, `isActive`, `parentIndentId`, `currentStatus`), search/history/cancellation DTOs.
  - Versioning endpoints:
    - `POST /api/indents` — create (v1, full validation + budget + workflow).
  ...
  - Draft endpoints (added):
    - `POST /api/indents/draft` — save new draft; no validation; generates real indent ID; `currentStatus=DRAFT`.
    - `PUT /api/indents/draft?indentId={id}` — update existing draft in-place; guards on DRAFT status + createdBy.
    - `POST /api/indents/draft/submit?indentId={id}` — promote draft to submitted; runs full validation + budget + workflow.
    - `GET /api/indents/drafts?userId={id}` — list all drafts for a user.
...
- Key modules/features:
  ...
  - Budget lifecycle management: hold at indent submit → check at PO/SO create → finalize hold/spend at PO/SO final approval → convert to spend at GRN approval → release on permanent GI rejection / cancellation / rejection. Tracked via `BudgetLedger` per reference.
  - Draft save pattern: implemented for Indent — skips validation and workflow, generates a real ID, stores as `currentStatus=DRAFT`. Fully reusable for Tender/PO/SO — see Section 7.
...
- Budget lifecycle risks:
  - `BudgetServiceImpl` depends on many repos across inventory and procurement modules — high coupling. Consider splitting into `IndentBudgetService`, `PoBudgetService`, `GrnBudgetService` if it grows further.
  - GRN and GI budget errors are swallowed (logged only) — budget figures can silently drift if chain resolution (GRN → GI → GPRN → PO) fails. Add monitoring/reconciliation job.
  - PO cancel logic (restoring indent holds) is not yet hooked — budget will remain in PO hold state after cancel until implemented.
  - `getReleasedIndentHoldsForTender` fetches ALL RELEASED indent ledger entries for a tender — if an indent was cancelled and re-submitted multiple times, this could double-count. Add a `cancelledAt` or `releasedReason` field to `BudgetLedger` for disambiguation.
  - `onHoldAmount` uses `.max(BigDecimal.ZERO)` guard in GRN/GI paths to prevent negative hold — indicates partial GRN amounts may slightly exceed PO hold if rounding differs. Monitor for drift.
...
- Draft-related risks (added):
  - `mapToResponseDTO` must be null-safe for all optional fields because draft rows bypass validation. Any new field added to `mapToResponseDTO` that uses stream operations (findFirst, reduce, map) must guard against null — follow the existing null-safety fix pattern.
  - Phantom drafts can accumulate if the API returns a 500 before the response reaches the frontend (DB insert succeeds, frontend never gets the ID back, next save creates a new draft). Monitor `indent_creation` where `currentStatus=DRAFT` for orphaned rows. The `GET /api/indents/drafts?userId=` endpoint lets the UI show and resume all drafts.
  - Draft IDs follow the same sequence as production IDs — there is no visual distinction in the ID itself. The `currentStatus=DRAFT` field and the `[DRAFT]` label in the search dropdown are the only indicators.
...
## 7. Draft Save Pattern — Replication Guide for AI

> This section describes the complete, tested draft pattern implemented for Indent.
> Follow these steps exactly to add draft saving to any other procurement module (Tender, PO, SO, etc.).
> Replace `Indent`/`indent`/`IND` with the target entity name throughout.

---

### 7.1 What "draft" means in this system

- A draft is a **real database row** with a real entity ID (e.g. `TND1005`), but with `currentStatus = "DRAFT"`.
- It is **invisible to approvers and workflow** — workflow is only initiated on final submit.
- It is **not validated** at save time — user can save partial/empty forms.
- On re-save, the **same row is updated in-place** (no new version, no new ID) via `orphanRemoval=true` collection replacement.
- On submit, the draft row is **promoted to submitted** — full validation runs, budget is held, workflow is initiated.

---

### 7.2 Backend — Entity changes

No schema changes needed beyond what already exists for the entity. Just confirm:
- `currentStatus` field exists (String). If not, add it.
- The entity's child collections (e.g. `materialDetails`, `jobDetails`) have `cascade=ALL, orphanRemoval=true` — this enables safe in-place replacement of line items on draft update.

---

### 7.3 Backend — Service methods to add

Add the following four methods to `XxxServiceImpl` (e.g. `TenderRequestServiceImpl`):

```
saveXxxDraft(XxxRequestDTO dto):
  - Generate entity ID using the same ID generator as createXxx.
  - Map all fields from DTO to entity (identical mapping as createXxx).
  - Set currentStatus = "DRAFT", isActive = true, version = 1.
  - Do NOT call BudgetService.
  - Do NOT call WorkflowService.
  - Save with saveAndFlush.
  - Return mapToResponseDTO(...) — must be null-safe (see 7.4).

updateXxxDraft(String xxxId, XxxRequestDTO dto):
  - Find existing row by xxxId. Throw BusinessException if not found.
  - Guard: currentStatus must be "DRAFT". Throw if not.
  - Guard: createdBy must match dto.getCreatedBy(). Throw if not.
  - Update all header fields in-place (same row, same ID).
  - Clear child collections: existing.getChildItems().clear()
  - Re-add child items from dto, setting parent reference: item.setXxx(existing); existing.getChildItems().add(item).
  - Do NOT call BudgetService or WorkflowService.
  - saveAndFlush.
  - Return mapToResponseDTO(...).

submitXxxDraft(String xxxId, XxxRequestDTO dto):
  - Find existing DRAFT row by xxxId. Throw if not found or not DRAFT.
  - Run all business validations (same as createXxx: vendor count, price cap, etc.).
  - Call BudgetService.holdBudgetForXxx(...) — blocks if insufficient.
  - Set currentStatus = "SUBMITTED" (or whatever the normal submitted status is).
  - Update any other fields from dto (user may have edited during draft).
  - saveAndFlush.
  - Return mapToResponseDTO(...).
  - NOTE: WorkflowService.initiateWorkflow is called by the CONTROLLER after this method returns (same pattern as createXxx).

getUserXxxDrafts(Integer userId):
  - Return xxxRepository.findByCreatedByAndCurrentStatus(userId, "DRAFT").
  - Order by createdDate desc.
```

---

### 7.4 Backend — null-safety in mapToResponseDTO

Because draft rows may have null values in fields that are always populated on a validated submit, `mapToResponseDTO` must guard against null before any stream operations:

**Rule 1 — findFirst on a mapped stream:**
```java
// WRONG — throws NPE if mapped value is null
.map(Entity::getSomeField).findFirst().orElse(null)

// CORRECT
.map(Entity::getSomeField).filter(Objects::nonNull).findFirst().orElse(null)
```

**Rule 2 — reduce with BigDecimal::add:**
```java
// WRONG — throws NPE if any element is null
.map(DTO::getTotalPrice).reduce(BigDecimal.ZERO, BigDecimal::add)

// CORRECT
.map(DTO::getTotalPrice).map(p -> p != null ? p : BigDecimal.ZERO).reduce(BigDecimal.ZERO, BigDecimal::add)
```

Apply these guards to every stream-based computation in `mapToResponseDTO` for the target entity. Check especially: subCategory/mode lookups, total price reductions, and any `Optional.of()` call.

---

### 7.5 Backend — Controller methods to add

Add to `XxxController` (e.g. `TenderRequestController`):

```java
@PostMapping("/draft")
public ResponseEntity<Object> saveXxxDraft(@RequestBody XxxRequestDTO dto) {
    XxxResponseDTO response = xxxService.saveXxxDraft(dto);
    return new ResponseEntity<>(ResponseBuilder.getSuccessResponse(response), HttpStatus.OK);
}

@PutMapping("/draft")
public ResponseEntity<Object> updateXxxDraft(
        @RequestParam String xxxId,
        @RequestBody XxxRequestDTO dto) {
    XxxResponseDTO response = xxxService.updateXxxDraft(xxxId, dto);
    return new ResponseEntity<>(ResponseBuilder.getSuccessResponse(response), HttpStatus.OK);
}

@PostMapping("/draft/submit")
public ResponseEntity<Object> submitXxxDraft(
        @RequestParam String xxxId,
        @RequestBody XxxRequestDTO dto) throws JsonProcessingException {
    XxxResponseDTO response = xxxService.submitXxxDraft(xxxId, dto);
    // Initiate workflow — same as createXxx
    workflowService.initiateWorkflow(response.getXxxId(), "Xxx Workflow", dto.getCreatedBy());
    return new ResponseEntity<>(ResponseBuilder.getSuccessResponse(response), HttpStatus.OK);
}

@GetMapping("/drafts")
public ResponseEntity<Object> getUserDrafts(@RequestParam Integer userId) {
    List<XxxResponseDTO> drafts = xxxService.getUserXxxDrafts(userId);
    return new ResponseEntity<>(ResponseBuilder.getSuccessResponse(drafts), HttpStatus.OK);
}
```

Note: `@RequestParam` on `xxxId` is mandatory — consistent with all other versioned ID endpoints.

---

### 7.6 Frontend — Page (e.g. Tender.jsx)

Add the following state and handler:

```javascript
const [draftBtnLoading, setDraftBtnLoading] = useState(false);

const handleSaveDraft = async () => {
    const payload = { ...formData, /* same payload construction as onFinish */ };

    try {
        setDraftBtnLoading(true);
        let response;

        if (formData?.xxxId && formData?.currentStatus === "DRAFT") {
            // Update existing draft in-place
            response = await axios.put(`/api/xxx-endpoint/draft`, payload, {
                params: { xxxId: formData.xxxId }
            });
            message.success("Draft updated successfully");
        } else if (!formData?.xxxId) {
            // Save new draft
            response = await axios.post(`/api/xxx-endpoint/draft`, payload);
            message.success("Draft saved successfully");
        } else {
            message.warning("This record is already submitted and cannot be saved as a draft.");
            return;
        }
...
In `onFinish`, add a draft-submit branch at the top of the submit routing:

```javascript
if (formData?.xxxId && formData?.currentStatus === "DRAFT") {
    response = await axios.post(`/api/xxx-endpoint/draft/submit`, payload, {
        params: { xxxId: formData.xxxId }
    });
...
Add a "Saved Draft" alert banner in JSX when `formData.currentStatus === "DRAFT"`:
```jsx
{formData?.xxxId && formData?.currentStatus === "DRAFT" && (
    <Alert
        message="Saved Draft"
        description="This record is a saved draft. It has not been submitted."
        type="warning"
        showIcon
    />
)}
...
In the search dropdown label mapping, append `[DRAFT]` suffix:
```javascript
label: item.currentStatus === 'DRAFT' ? `${item.xxxId}  [DRAFT]` : item.xxxId
```

---

### 7.7 Frontend — ButtonContainer

Pass the two new props to `ButtonContainer`:
```jsx
<ButtonContainer
    ...existingProps...
    onDraft={handleSaveDraft}          {/* API handler — no localStorage */}
    draftBtnLoading={draftBtnLoading}  {/* loading spinner state */}
/>
```

`ButtonContainer` is already updated to use `onDraft` via `onClick` (no `htmlType="submit"`) and `draftBtnLoading` for the spinner. No changes needed to the component itself.

---

### 7.8 Checklist for a new module

- [ ] Confirm `currentStatus` field on entity.
- [ ] Confirm child collection has `cascade=ALL, orphanRemoval=true`.
- [ ] Add `saveXxxDraft`, `updateXxxDraft`, `submitXxxDraft`, `getUserXxxDrafts` to service.
- [ ] Apply null-safety guards to `mapToResponseDTO` for all stream operations.
- [ ] Add four controller endpoints (`POST /draft`, `PUT /draft`, `POST /draft/submit`, `GET /drafts`).
- [ ] Add `handleSaveDraft` function and `draftBtnLoading` state to the page.
- [ ] Add draft-submit branch to `onFinish`.
- [ ] Pass `onDraft` and `draftBtnLoading` to `ButtonContainer`.
- [ ] Add "Saved Draft" alert banner in JSX.
- [ ] Add `[DRAFT]` label to search dropdown.
- [ ] (Optional) Add `GET /drafts?userId=` call in a draft picker / resume drafts UI.
...
- Section 7 (Draft Save Pattern) is the canonical reference for adding draft behavior to any procurement module. Update it if the pattern changes.
