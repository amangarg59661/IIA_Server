...
│   │   │     - Purpose: create/update/search/assign/cancel indents, history, cancellation requests.
│   │   │     - Key methods: `createIndent`, `updateIndent`, `getIndentDataForTenderById`, `searchIndents`, `assignEmployee`, `requestIndentCancellation`, `approveCancellationRequest`.
...
│   │   │     - Purpose: build indent aggregate, validate mode/vendor count, save files, compute value, cancellation flow.
│   │   │     - Key methods: `createIndent`, `updateIndent`, `searchIndentIds`, `assignEmployeeToIndent`, `requestIndentCancellation`, `approveCancellationRequest`.
│   │   │     - Dependencies: indent/material/job repos, workflow repo, vendor/job mapping repo, user/employee repos, PO repos, email.
│   │   │     - Transaction boundaries: `createIndent`, `requestIndentCancellation`, `approveCancellationRequest`.
...
│   │   │     - Key methods: `saveGrn`, `getGrnDtls`, `approveGrn`, `rejectGrn`, `changeReqGrn`, `saveMaterialGrn`, `getPendingGrns`.
│   │   │   - `GiServiceImpl`
│   │   │     - Purpose: GI save/approval/rejection/change-request/history/pending dropdowns.
│   │   │     - Key methods: `saveGi`, `getGiDtls`, `approveGi`, `rejectGi`, `changeReqGi`, `getGiByStatuses`, `getPendingGprnsForGI`.
│   │   │   - `GtServiceImpl`
...
│   │   │     - Dependencies: `/api/tender-requests*`, `/api/indents/IndentDataForTender/*`, `/api/location-master`, `/getApprovedTenderIdForPOAndSO`.
...
- GRN:
  ...
  - Repos: `GrnMasterRepository`, `GrnMaterialDtlRepository`, `GrnWorkflowStatusRepository`, PO/GI support repos.
  - Entities: `GrnMasterEntity`, `GrnMaterialDtlEntity`, `GrnWorkflowStatus`.
...
- GI:
  ...
  - Repos: `GiMasterRepository`, `GiMaterialDtlRepository`, `GiWorkflowStatusRepository`.
...
- Tender flow:
  - `Tender.jsx` -> `CustomForm` + print modal -> `/api/tender-requests*` + `/api/indents/IndentDataForTender/*`.
...
- `GET/POST/PUT /api/indents*`
  ...
  - Response: `IndentCreationResponseDTO`, search/history/cancellation DTOs.
