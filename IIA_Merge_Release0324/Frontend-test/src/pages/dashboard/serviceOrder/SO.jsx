...
//     let budgetData = [];
//     if (data?.responseData) budgetData = data.responseData;
//     else if (data?.data) budgetData = data.data;
//     else if (Array.isArray(data)) budgetData = data;
//     const budgetOptions = budgetData.map((budget) => ({
...
//       };
//       return { ...baseRow, ...calcRowAmounts(baseRow) };
//     })
// );

//       setJobs(allJobs);
//       // Budget codes: project-specific list if the tender's primary indent carries a project, else full list
...
//         ...section,
//         children: section.children.map((child) => {
//           if (child.name === "jobCode") {
//             return { ...child, options: jobs };
//           }
//           if (child.name === "budgetCode") {
//             const hasProject = (formData?.jobDtlList || []).some((j) => j.projectName);
//             return {
//               ...child,
//               options: hasProject
...
//               ...child,
//               shouldShow: () =>
//                 (formData?.jobDtlList || []).some((j) => j.projectName != null),
//             };
//           }
//           return child;
//         }),
...
//   // Auto-calc inrEquivalent + totalAmount for a jobDtlList row
//   // TOP-LEVEL (sibling to handleChange) so every handler can reach it
//   const calcRowAmounts = (row) => {
//     const rate = Number(row.rate) || 0;
//     const exchangeRate = Number(row.exchangeRate) || 0;
//     const gst = Number(row.gst) || 0;
//     const duties = Number(row.duties) || 0;
//     const quantity = Number(row.quantity) || 0;
//     const currency = (row.currency || "").toString().toUpperCase();

//     const unitPrice = currency && currency !== "INR" ? rate * exchangeRate : rate;
//     const inrEquivalent = quantity * unitPrice;
//     const totalAmount =
//       inrEquivalent + (gst / 100) * inrEquivalent + (duties / 100) * inrEquivalent;

//     return {
//       inrEquivalent: Number(inrEquivalent.toFixed(2)),
...
//     if (["rate", "exchangeRate", "currency", "gst", "duties", "quantity"].includes(field)) {
//       updatedRow = { ...updatedRow, ...calcRowAmounts(updatedRow) };
//     }
...
//   }),
// });
// // Budget codes: project-specific list if the loaded SO carries a project, else full list
//       const soProjectName =
//         responseData?.projectName ||
//         (Array.isArray(responseData?.materials)
//           ? responseData.materials[0]?.projectName
//           : undefined);
//       if (soProjectName) {
//         fetchBudgetCodesByProject(responseData?.projectCode || soProjectName);
//       } else {
//         setProjectBudgetCodes([]);
//       }
//       setSearchDone(true);
...
const [allBudgetCodes, setAllBudgetCodes] = useState([]);
  ...
  // Fetch full budget code list (used when job has no project)
  const fetchAllBudgetCodes = async () => {
    try {
      const { data } = await axios.get('/api/admin/budget');
      const budgetData = data?.responseData || [];
  ...
  // Fetch budget codes scoped to a project (used when job has a project)
  const fetchBudgetCodesByProject = async (projectCode) => {
    if (!projectCode) {
      setProjectBudgetCodes([]);
      return;
