...
if (tenderResponse.isActive === false) {
  ...
  const buildPayload = (data = formData) => ({
    ...
    fileType: "Tender",
    materialDetails: (data.materialDetails || []).map((m) => ({
    ...
    jobDetails: (data.jobDetails || []).map((j) => ({
      jobCode: j.jobCode || "",
    ...
    ...(data.buyBack ? {
      ...
      uploadBuyBackFileNames: data.uploadBuyBackFileNames || "",
    } : {}),
  });

  // ──────────────────────────────────────────────────────────────────────
  // ROOT-CAUSE FIX:
  // Previously, `onFinish` was called directly by ButtonContainer / CustomForm
  // without ever running `form.validateFields()`. That meant AntD's `rules`
  // (including the `required: true` rules on openingDate/closingDate) were
  // defined but never actually evaluated before the API call fired.
  //
  // The actual submission logic below is unchanged — it has just been
  // renamed to `submitTender` and is now only ever invoked *after*
  // `form.validateFields()` resolves successfully (see `onFinish` further
  // down). This guarantees the API is never called while required fields
  // (including the two DatePickers) are empty or invalid.
  // ──────────────────────────────────────────────────────────────────────
  const submitTender = async (values) => {
  try {
    const currentData = {
      ...formData,
      ...values,
    ...
    // ── DRAFT SUBMIT BRANCH — runs before all other checks ──────────────────
    if (currentData?.tenderId && currentData?.currentStatus === "DRAFT") {
      const payload = buildPayload(currentData);
      setSubmitBtnLoading(true);
      const response = await axios.post(`/api/tender-requests/draft/submit`, payload, {
...
if (newTenderId) {
  ...
  // ──────────────────────────────────────────────────────────────────────
  // `onFinish` is now the single gatekeeper: it MUST run AntD's own
  // `validateFields()` against the `form` instance that is actually bound
  // to the rendered <Form.Item> fields (openingDate, closingDate, etc.)
  // before the real submission logic (`submitTender`) is allowed to run.
  //
  // IMPORTANT: for this to work, the `form` instance created here via
  // `Form.useForm()` must be the SAME instance bound to the rendered
  // <Form> inside <CustomForm>. That's why `form={form}` is now passed
  // down to <CustomForm> below — previously it was not passed at all,
  // which meant this component's `form` reference could be disconnected
  // from the actual rendered fields, silently breaking both required
  // rules and the cross-field opening/closing date validators.
  // ──────────────────────────────────────────────────────────────────────
  const handleFormSubmit = async (values) => {
    ...
    if (missingFields.length) {
      form.setFields(missingFields);
      form.scrollToField(missingFields[0].name, {
        behavior: "smooth",
        block: "center",
      });
      message.error("Please complete the required Start Date and Closing Date fields.");
      return;
    }
    ...
    try {
      ...
      console.warn("Validation failed:", validationError?.errorFields);
      return; // Stop here — submitTender/axios is never reached.
    }
  ...
  const onFinishFailed = (errorInfo) => {
    ...
    const errorMessage = firstError?.errors?.[0] || "Please check required fields before submitting.";
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      ...
      if (formData?.tenderId && formData?.currentStatus === "DRAFT") {
        // Update existing draft in-place
        response = await axios.put(`/api/tender-requests/draft`, payload, {
          params: { tenderId: formData.tenderId },
          headers: { Authorization: `Bearer ${token}` },
        });
        message.success("Draft updated successfully");
      } else if (!formData?.tenderId) {
        // Save as new draft
        response = await axios.post(`/api/tender-requests/draft`, payload, {
          headers: { Authorization: `Bearer ${token}` },
        });
        message.success("Draft saved successfully");
      } else {
        message.warning("This tender is already submitted and cannot be saved as a draft.");
        return;
      }

      const savedData = response?.data?.responseData;
      if (savedData?.tenderId) {
        setFormData(prev => ({
          ...prev,
 ...
 const handleSearchTenderIds = async () => {
  ...
  try {
    ...
    const dropdownOptions = tenderList.map((item) => ({
      label: item.currentStatus === "DRAFT" ? `${item.tenderId}  [DRAFT]` : item.tenderId,
      value: item.tenderId
    }));
      ...
      fieldList: [
        ...
        },
        {
          ...
          rules: [
            ...
            {
              validator: (_, value) => {
                const closingDate = form.getFieldValue("closingDate");
                if (!value || !closingDate) return Promise.resolve();
                if (value.isAfter(closingDate, "day")) {
                  return Promise.reject(new Error("Start Date cannot be after Closing Date"));
                }
          ...
          rules: [
            { required: true, message: "Please select Closing Date" },
            {
              validator: (_, value) => {
                ...
                if (!value || !openingDate) return Promise.resolve();
                if (value.isBefore(openingDate, "day")) {
                  return Promise.reject(new Error("Closing Date cannot be before Start Date"));
                }
                return Promise.resolve();
              },
            },
...
{formData?.tenderId && (
  <div style={{ marginBottom: 16 }}>
    ...
    <Button icon={<HistoryOutlined />} onClick={() => fetchVersionHistory(formData.tenderId)}>
      ...
      {/* Saved Draft Banner */}
      {formData?.tenderId && formData?.currentStatus === "DRAFT" && (
        <div style={{
          background: "#fffbe6",
          ...
          <span>
            <strong>Saved Draft</strong> — This tender ({formData.tenderId}) is a saved draft. 
            Complete the form and click <strong>Submit</strong> to send for approval.
          </span>
        </div>
      )}
      ...
      {/* Form Start */}
      {/*
        ROOT CAUSE (confirmed): DKG_CustomForm accepts the shared form
        instance under the prop name `customForm`, not `form`. Passing
        `form={form}` was silently ignored by DKG_CustomForm, so it fell
        back to its own internally-created Form.useForm() instance and
        bound THAT to the rendered <Form>. Meanwhile every validateFields()/
        getFieldValue() call in this file was checking the orphaned
        instance created here — which had zero fields registered to it —
        so required-field and cross-field date validation always silently
