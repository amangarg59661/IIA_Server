...
{/* }; */}
    ...
    >
      {(() => {
        ...
        // Prefer the workflowId captured from the row record when version
        // history was requested -- the version-history API response itself
        // does not reliably include a workflowId on each record (confirmed:
        // the indent endpoint omits it), so falling back to it produced an
        // unresolved wId and silently empty line-item diffs.
        const wId = parseInt(
          versionHistoryWorkflowId ?? curr.workflowId ?? versionHistoryList[0]?.workflowId,
          10
        );
