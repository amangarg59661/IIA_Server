        ...
        for (GiMaterialDtlDto gmdd : req.getMaterialDtlList()) {
            ...
            if (shouldCreateAsset) {
                Optional<GiMaterialDtlEntity> gimdeOpt = gimdr.findByGprnSubProcessIdAndMaterialCode(
                        extractSubProcessId(req.getGprnNo()), gmdd.getMaterialCode());
                ...
                if (gimdeOpt.isPresent()) {
                    ...
                    errorFound = true;
                    continue;
                } else if (gmdd.getReceivedQuantity()
                ...
                for (int i = 0; i < qty; i++) {
                    ...
                    // First row carries the rejected quantity, all subsequent rows get zero
                    if (i == 0) {
