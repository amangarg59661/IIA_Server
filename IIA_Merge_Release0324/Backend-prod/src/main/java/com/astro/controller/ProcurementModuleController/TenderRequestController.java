...
// // POST /api/tender-requests/draft — save new draft
// @PostMapping("/draft")
// public ResponseEntity<Object> saveTenderDraft(@RequestBody TenderRequestDTO dto) {
//     TenderResponseDTO saved = tenderRequestService.saveTenderDraft(dto);
//     return new ResponseEntity<>(ResponseBuilder.getSuccessResponse(saved), HttpStatus.CREATED);
// }

// // PUT /api/tender-requests/draft?tenderId= — update existing draft in-place
// @PutMapping("/draft")
// public ResponseEntity<Object> updateTenderDraft(
//         @RequestParam String tenderId,
//         @RequestBody TenderRequestDTO dto) {
//     TenderResponseDTO updated = tenderRequestService.updateTenderDraft(tenderId, dto);
//     return new ResponseEntity<>(ResponseBuilder.getSuccessResponse(updated), HttpStatus.OK);
// }

// // POST /api/tender-requests/draft/submit?tenderId= — promote draft to submitted
// @PostMapping("/draft/submit")
// public ResponseEntity<Object> submitTenderDraft(
//         @RequestParam String tenderId,
//         @RequestBody TenderRequestDTO dto) {
//     TenderResponseDTO submitted = tenderRequestService.submitTenderDraft(tenderId, dto);
//     // Initiate workflow — same as createTenderRequest
//     workflowService.initiateWorkflow(submitted.getTenderId(), "Tender Approver Workflow", dto.getCreatedBy());
//     return new ResponseEntity<>(ResponseBuilder.getSuccessResponse(submitted), HttpStatus.OK);
// }

// // GET /api/tender-requests/drafts?userId= — list user's drafts
// @GetMapping("/drafts")
// public ResponseEntity<Object> getUserTenderDrafts(@RequestParam Integer userId) {
//     List<TenderResponseDTO> drafts = tenderRequestService.getUserTenderDrafts(userId);
...
// POST /api/tender-requests/draft — save new draft
@PostMapping("/draft")
public ResponseEntity<Object> saveTenderDraft(@RequestBody TenderRequestDto tenderRequestDTO) {
    TenderResponseDto saved = TRService.saveTenderDraft(tenderRequestDTO);
    return new ResponseEntity<>(ResponseBuilder.getSuccessResponse(saved), HttpStatus.CREATED);
}
...
public ResponseEntity<Object> updateTenderDraft(
    ...
    TenderResponseDto updated = TRService.updateTenderDraft(tenderId, tenderRequestDTO);
    return new ResponseEntity<>(ResponseBuilder.getSuccessResponse(updated), HttpStatus.OK);
}
...
public ResponseEntity<Object> submitTenderDraft(
        @RequestParam String tenderId,
        @RequestBody TenderRequestDto tenderRequestDTO) {
    TenderResponseDto submitted = TRService.submitTenderDraft(tenderId, tenderRequestDTO);
    workflowService.initiateWorkflow(
            submitted.getTenderId(),
            WorkflowName.TENDER_APPROVER.getKey(),
            submitted.getCreatedBy());
    return new ResponseEntity<>(ResponseBuilder.getSuccessResponse(submitted), HttpStatus.OK);
}
...
@GetMapping("/drafts")
public ResponseEntity<Object> getUserTenderDrafts(@RequestParam Integer userId) {
    List<TenderResponseDto> drafts = TRService.getUserTenderDrafts(userId);
    return new ResponseEntity<>(ResponseBuilder.getSuccessResponse(drafts), HttpStatus.OK);
}
    @PutMapping(value = "/update")
    public ResponseEntity<Object> updateTender(@RequestParam String tenderId, @RequestBody tenderUpdateDto tenderRequestDTO) {// Set files in DTO if provided
